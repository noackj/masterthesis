...
\end{figure}
Ein mögliches Vorgehen bei der Modellierung von Geschäftsprozessen wird in \citep[vgl.][S. 13-17]{Jacka2009} beschrieben und besteht aus den Schritten Prozessidentifizierung, Datenerfassung, Interview und Modellierung, Datenanalyse und Präsentation. Demzufolge ist die Prozessidentifizierung der Einstieg und besteht daraus, auf abstrakter Ebene die zu modellierenden Prozesse zu benennen. Da dies den Grundstein für die folgende Modellierung legt, ist hierbei besondere Sorgfalt angebracht. Methodisch hilf es hierbei zunächst die möglichen auslösenden Ereignisse (engl. "`Trigger Events"') zu ermitteln und aus diesen jeweils einen Prozess abzuleiten. Das Ergebnis der Überlegungen ist in Tabelle \ref{tab:trigger_events} abgebildet.
\begin{table}[ht]
\begin{center}
...
\end{center}
\end{table}
Demzufolge müssten im Zuge der Modellierung mindestens 5 einzelne Prozesse ermittelt werden können.

Die nächsten beiden Schritte Datenerfassung und Interview beschäftigen sich mit der Problemstellung, die notwendigen Daten zur Prozessmodellierung zu sammeln. Wie bereits erwähnt wurde, kann hier auf die Unterlagen des QM-Handbuches zurückgegriffen werden und die Interviews eher als Abgleich und Sicherstellung des Verständnisses für die gewonnen Daten gesehen werden.

- Beschreibung des Vorgehens, laut Anweisungen im Buch (--> OneNote Tabelle mit Identifikation der Prozessteile)
- Beschreibung der Modellierungstiefe (die 
- nach Abschluss der Modellierung nach den Unterlagen wird im Rahmen von Meetings die Konformität zwischen Prozessbeschreibungen und Modell sichergestellt (Treffen mit dem Fachbereichsleiter Qualitätsmanagement, der die Hauptverantwortung über die bestehenden Teile des Handbuchs besitzt, und Mitarbeitern aus Regulatory Affairs, die speziell bezogen auf die normativen Anforderungen und Meldepflichten die entscheidende Instanz bei WOM darstellen
