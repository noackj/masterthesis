...
- MDR ist Verordnung des Europäischen Rates und des Rates und besitzt somit einen gesetzesähnlichen Charakter
- die Ermittlung, ob alle Anforderungen eingehalten werden, gestaltet sich somit schwierig (Artikel nicht prozessorientiert aufgearbeitet für Unternehmen, sondern thematisch orientiert mit Querverweisen und Redundanzen)
- thematisch zusammenhängende Sachverhalte sind häufig über mehrere Artikel verteilt, was es erschwert, den Zusammenhang zu erfassen und somit das große Bild wahrzunehmen
- zudem sind nicht alle Informationen für WOM relevant, da beispielsweise keine entsprechenden Geräte im Portfolio bestehen oder Anforderungen an die zuständigen Behörden formuliert werden
- Verständnis und Nachvollziehbarkeit muss gegeben sein und unter Prämisse systematisch vorzugehen wurde beschlossen vor der eigentlichen Einschätzung und Analyse der Prozesse bei WOM auf Deckungsgleichheit mit den Forderungen der MDR eine ausgiebige Analyse der MDR vorzuschalten, indem jeder relevante Artikel auf seine Relevanz für WOM komprimiert wird und in Anforderungsform überführt wird
- dazu wurde zunächst jeder Artikel analysiert und in Anforderungen an die Unternehmensprozesse bei WOM überführt
- ein Beispiel für dieses Vorgehen kann in (Referenz Artikel 84) gesehen werden, wo der tatsächliche Inhalt der MDR den daraus extrahierten Anforderungen gegenübergestellt ist
- um die Nachvollziehbarkeit und das Verständnis zu erhöhen werden diese ermittelten Anforderungen in einen zusammenfassenden Liste vereint
- dabei wird über Gruppierung/Clusterung von Anforderungen und Auslöschung von Redundanzen (z.B. in Form von Verweisen und Wiederholungen) eine zusammenfassende Anforderungsliste generiert, die die Analyse der Unternehmensprozesse erleichtern sollte
- das Ergebnis ist eine Checkliste, mit der der Erfüllungsgrad transparent festgestellt werden kann
- Das Ergebnis ist in Tabelle (Referenz einfügen) zu erkennen
- über die zweite Spalte ist es weiterhin möglich nachzuvollziehen, aus welchen Artikeln und Absätzen die einzelnen Anforderungen abgeleitet wurden, was die Transparenz und Nachvollziehbarkeit sicherstellt und somit den Nutzen für WOM maximiert

\def\requestListTabHead{\begin{table}[ht] \centering \begin{tabular}{|p{0.87\textwidth}|p{0.07\textwidth}|} \hline\vspace{0mm} Anforderung & \vspace{0mm}Art.\\ \hline\hline}
\def\ListTabBottom{\end{tabular}\end{table}}
\def\plainTabularText#1#2{\begin{tabular}{p{14,1cm}}#1 \\\end{tabular} & \begin{tabular}{p{1cm}}#2 \\\end{tabular}\\}
\def\tabularitemizeFirstLayer#1#2{\begin{tabular}{p{0,3cm}p{13,4cm}}\textbullet & #1 \\\end{tabular} & \begin{tabular}{p{1cm}}#2 \\\end{tabular}\\}
\def\tabularitemizeSecondLayer#1#2{\begin{tabular}{p{0,3cm}p{0,3cm}p{12,7cm}}& - & #1 \\\end{tabular} & \begin{tabular}{p{1cm}}#2 \\\end{tabular}\\}
...
- nachdem ein Anforderungskatalog aus der MDR extrahiert wurde, können die aktuellen Prozesse analysiert werden, wobei die Erfüllung jeder einzelnen Anfrage geprüft wird
- Ergebnis dieser Analyse ist im Anhang (Referenz) zu sehen
- es zeigt sich, dass nicht alle Anforderungen erfüllt werden, was aufgrund komplett neuer Anforderungen logisch erscheint
- das elektronische System, was von den Behörden als zentrale Informationsplatform zur Verfügung gestellt wird und die Verwendung dessen stellen eine der Hauptquellen für nicht erfüllte Anforderungen dar, da diese allesamt neu sind
- des weiteren wurden die Fristen des Meldesystems der Hersteller deutlich nachgeschärft, wodurch neue striktere Fristen eingeführt werden, die auch noch nicht erfüllt werden
- generell gilt, dass die Behörden eine wichtige Rolle in der MDR spielen, wodurch einige Anforderungen sich aus den möglichen Aktionen der Behörden ergeben. Diese können beispielsweise Korrektumaßnahmen oder Untersuchungen einfordern, was in den aktuellen Prozessen noch nicht berücksichtigt ist
- speziell die passive Interaktion mit den Behörden (z.B. Reaktion auf Anfragen) ist in den momentanen Prozessen nicht erschlossen, was in der Praxis zu Abstimmungsschwierigkeiten führen könnte, da nicht genau geregelt ist, welche Stelle sich um die Bearbeitung einer Anfrage von außen kümmern soll
- ebenfalls nicht umgesetzt sind die Meldung von Trends und die Erstellung eines Plans für die Aktivitäten zur Marktbeobachtung nach dem Inverkehrbringen
- letzteres spielt zwar eine wichtige Rolle, da durch den Plan der Grundstein der Überwachung gelegt wird, wird aber in dieser Arbeit nicht betrachtet, da dieser Prozess noch vor dem Inverkehrbringen abläuft und somit nicht im Betrachtungshorizont dieser Arbeit liegt
...
\hline
\plainTabularTextWOCheckbox{Es werden Daten über Qualität, Leistung und Sicherheit eines Produktes während gesamter Lebensdauer gesammelt, aufgezeichnet, analysiert und verwendet für:}
